...
{
	GradeType  grades;
	// the array holding thegrades.     
	int  numberOfGrades;
	// the number of grades read.
	int pos;
	// index to the array.
	float avgOfGrades;
	// contains the averageof the grades.
	int highestGrade;
	...
	while (!infile.eof())
	{
		for (pos = 0; pos < 100 && (!infile.eof()); pos++)
		{
			infile >> grades[pos];
		}
	}
...
//                the array // data in:       array of floating point numbers 
...
//Excercise 2: The output is: The average grade is 67.5, the highest is 90, and lowest is 45
